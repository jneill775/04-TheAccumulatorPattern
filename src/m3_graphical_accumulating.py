...
def draw_parallel_lines(n, point, length, window):
    ...
    for k in range(n):
        iansmith = rg.Line(rg.Point(point.x, point.y + (30 * k)), rg.Point(point.x + length, point.y + (30 * k)))
        iansmith.attach_to(window)
    window.render()
    """
...
def draw_lines(n, point, window):
    ...
    for k in range(n - 1):
        line = rg.Line(point, rg.Point(point.x + 100, ((200 / n-1) * k) + (point.y - 100)))
