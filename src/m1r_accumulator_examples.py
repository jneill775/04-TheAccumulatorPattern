...
#
#              for k in range(r - s): <--- r and s are reversed in their order, should be s - r
#                  count = 0
#                  if math.cos(r) > 0: <---
#                      count = 1 <--- should be count = count + 1
#                  return count <--- should be indented outside of the for loop
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean?
#                   it is k, the entity that does the counting from 0 to n
#
...
#              x = starting_point.x
#              for k in range(n):
#                  center = rg.Point(x + (k * diameter), y)     <-----THIS ONE IS WRONG
#                  circle = rg.Circle(point, radius)
#                  circle.attach_to(window)
#
#              x = starting_point.x
